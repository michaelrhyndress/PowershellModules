...
#region Invoke PsUtils.CredentialManager
$PsCredMan = $null
Try{
	$PsCredMan = [PsUtils.CredentialManager]
} Catch{}
...
#endregion Invoke PsUtils.CredentialManager


#region Load Global Functions
$GlobalModules = @("../Protect-String")

foreach( $GlobalModule in $GlobalModules) {
    Import-Module (`
        Get-Item (`
            [System.IO.Path]::GetFullPath((`
                Join-Path $PSScriptRoot $GlobalModule)`
            )`
        )`
    )
}
#endregion Load Global Functions


#region Load public and private functions
...
{
    Try {
        . $import.fullname #Dot source the files
    }
    Catch {
        Write-Error -Message "Failed to import function $($import.fullname): $_"
    }
}
#endregion Load public and private functions


$Script:ErrorCategory = [HashTable]@{0x80070057 = "InvalidArgument";
