...
Describe "Compare-Hashtable" {
  ...
  Context "When both contain arrays" {
    $Left = @{ a = @(1,2,3,4); b = @(1,2,3,4) }
    $Right = @{ b = @(1,2,3,4)}
    $Result = Compare-Hashtable $Left $Right

    It "should contain 1 differences" {
      $Result.Length | Should Be 1
    }
    It "should return a: 1 <=" {
      ($Result | ? { $_.Key -eq "a" }).Side   | Should Be "<="  
      ($Result | ? { $_.Key -eq "a" }).LValue | Should Be @(1,2,3,4)  
      ($Result | ? { $_.Key -eq "a" }).RValue | Should Be $Null  
    }
    It "should return nothing b: @(1,2,3,4)" {
      ($Result | ? { $_.Key -eq "b" }).Side   | Should BeNullOrEmpty
    }
  } 
}
