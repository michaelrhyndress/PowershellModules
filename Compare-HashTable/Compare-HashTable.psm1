...
[CmdletBinding()]
    ...
    [Object[]]$Results = $Left.Keys | % {
        if ($Left.ContainsKey($_) -and !$Right.ContainsKey($_)) {
            ...
            #For array
            If ($LValue -is [array] -and $RValue -is [array]) {
                If ($LValue | ?{$_ -notin $RValue}) {
                    New-Result $_ $LValue "!=" $RValue
                }
            }
            Else { #Strings, int, and other
                if ($LValue -ne $RValue) {
                    New-Result $_ $LValue "!=" $RValue
                }
            }
...
New-Alias -Name chash -Value Compare-Hashtable
Export-ModuleMember -Function Compare-Hashtable
